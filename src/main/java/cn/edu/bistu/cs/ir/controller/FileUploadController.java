...
public class FileUploadController {
    ...
    /**
     * 本地AI图片分析接口 - 接收Postman请求并调用Python微服务
     */
    @PostMapping("/analyze-local")
    public ResponseEntity<Map<String, Object>> analyzeLocalImage(
            @RequestParam("image") String imageBase64,
            @RequestParam(value = "prompt", required = false, defaultValue = "请分析这张图片") String prompt) {
    ...
    /**
     * 调用Python微服务 - 按照已有规范调用 http://127.0.0.1:5000/analyze
     */
